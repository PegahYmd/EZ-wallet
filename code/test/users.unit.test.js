...
describe('getUser', () => {
  ...
  test("should return user data for authenticated user requests", async () => {
  ...
  test("should return user data for authenticated admin requests", async () => {
  ...
  test("should return 400 if user is not found as admin", async () => {
    const req = mockReq('nonExistentUser');
    const res = mockRes();
...
describe("createGroup", () => { 
  ...
  test ('400 - one or more mails are empty strings', async()=> {
    //mock auth
  ...
  test ('400 - one or more mails are not in a valid format', async()=> {
    //mock auth
  ...
  test ('400 - all users already in group or do not exist', async()=> {
    ...
    User.findOne = jest.fn().mockResolvedValue(null); //calling users doesn't exist
    const mockReq = {
        body: {name: "test",
              memberEmails:[
                "callingUser@email.com",
                "test@email.com",
              ]},
        cookies: "accessToken=sampleWrongAccessToken;refreshToken=sampleWrongRefreshToken"
    };
  ...
  test ('200 - Group created correctly', async()=> {
    ...
    expect(mockRes.json).toHaveBeenCalledWith({
      data:{ group: expectedGroup , EmailsNotExisting : [], EmailsAlreadyUsed: []},
      refreshedTokenMessage: mockRes.locals.refreshedTokenMessage
  });
  });
})

describe("getUser", () => {// This represents the user in the database
  const dbUser = {
    username: 'user1',
    email: 'user1@test.com',
    role: 'Regular'
...
describe("getGroups", () => { 
  ...
  test ('Admin gets all groups', async()=> {
    verifyAuth.mockReturnValue({authorized:true, cause:"Authorized"});
  ...
  test ('Admin gets []: no group exists', async()=> {
    verifyAuth.mockReturnValue({authorized:true, cause:"Authorized"});
...
describe("getGroup", () => { 
  ...
  test('Returns the required group', async()=> {
    verifyAuth.mockReturnValue({authorized:true, cause:"Authorized"});
...
describe("addToGroup", () => {
  ...
  test ('Successfully inserted', async()=> {
    ...
    await addToGroup(mockReq,mockRes);
    expect(mockRes.status).toHaveBeenCalledWith(200);
    expect(mockRes.json).toHaveBeenCalledWith({data:{ group: expectedResult , EmailsNotExisting : [], EmailsAlreadyUsed: []},
...
describe("removeFromGroup", () => {
  ...
  test ('Succesfully deleted members', async()=> {
  ...
  test ('Succesfully deleted members - keep the first because #members == #emails', async()=> {
...
describe('deleteUser', () => {
  ...
  test(" elete a user and return the number of deleted transactions and whether the user was deleted from a group", async () => {
  ...
  test("cannot delete admin error 400", async () => {
  ...
  test("should return 400 if email format is empty string", async () => {
    const req = mockReq(' ');
    const res = mockRes();
    verifyAuth.mockImplementation(() => { return { authorized: true } });
    await deleteUser(req, res);
...
describe("deleteGroup", () => {
  ...
  test ('The group is deleted by the admin', async()=> {
    verifyAuth.mockReturnValue({authorized:true, cause:"Authorized"});
