    ...
    export const createGroup = async (req, res) => {
      try {
        ...
        return res.status(200).json({
              data:{ group: print_group , EmailsNotExisting : arrayEmailNot, EmailsAlreadyUsed: arrayEmailAlready},
...
export const addToGroup = async (req, res) => {
  try {
    ...
    if(!req.params.name || req.params.name.length === 0 || !req.body.emails || req.body.emails.length === 0)
      ...
      return res.status(200).json({
        data:{ group: group , EmailsNotExisting : arrayEmailNot, EmailsAlreadyUsed: arrayEmailAlready},
        refreshedTokenMessage: res.locals.refreshedTokenMessage
      })
